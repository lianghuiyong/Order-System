...
void cust_delete(char *cust_name_delete)
{
	...
	sprintf(sql,"update tbname set condition = 0 where id1 = %d;",tb_id);
	sqlite3_exec(db,sql,0,0,&errmsg);
	
	//sprintf(sql,"update tbname set condition = 0 where id1 = (select tb_id1 from %s where cust_name1 like '%s');",tb_id,cust_name_delete,cust_name_delete);
	//sqlite3_exec(db,sql,0,0,&errmsg);
