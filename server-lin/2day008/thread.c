	...
	case 22:
		{
			...
			char *errmsg;
			char sql[BUFSIZ];
			sprintf(sql,"select * from cli_id_%d",cli_fd);
			/*search db*/
			...
			const char * buf_cst =json_object_to_json_string(json_shuzu);
			/*call cook
			 * input the msg to be cooked*/
			msg.msg_type = 1;
			strncpy(msg.msg_text,buf_cst,strlen(buf_cst));
...
void *cook_fun(void *arg)
{
	...
	while(1){
		...
		char cust_name[20];
		sprintf(cust_name,"cli_id_%d",cli_fd);//delete the table of a client
		update_cust(cust_name);
		pthread_mutex_unlock(&mutex);
		printf("cook done %s\n",msg.msg_text);
